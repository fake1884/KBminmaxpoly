...
Das beiliegende R-Paket ist in der beiliegenden README Datei beschrieben.
...
In der Graphik \ref{Beispieldaten} sind die Datentupel $(Y,G)$ eingezeichnet:
...
Benutzt man die Formeln \eqref{beta} und \eqref{sigma} von oben erhält man  $\hat{\beta}$ = \betaoneest ~und $\widehat{\sigma^2}$ = \sigmaoneest. In der Graphik \ref{Beispieldaten_Regressionsgerade} sind die Daten mit der Regressionsgerade eingezeichnet.
...
Da bei der Berechnung des kritischen Wertes für das Konfidenzband auf $A$ eine Simulation verwendet wird und Reproduktivität gewährleisten sein soll, wird an dieser Stelle die Seed auf \seedsimulation ~gesetzt. 
